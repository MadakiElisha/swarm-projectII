...
from swarm_interfaces.msg import MissionEvent, MissionStatus
...
class TaskAuctioneer(Node):
    def __init__(self):
        ...
        self.suggestions = {}
        self._swarm_seen = set()
        self.winners = {}
        self.station = {}
        self.formed_state = {}
        self.event_pub = self.create_publisher(MissionEvent, '/swarm/mission_event', 10)
        self.create_subscription(MissionStatus, '/swarm/mission_status', self.on_status, 10)      # task_id -> first-seen (awaiting human confirmation)
    ...
    def on_status(self, m):
        ...
        if not winners or not getattr(self, 'is_leader', True):
            return
        now = time.time()
        st = lambda w: self.station.get((m.task_id, w), ('', 0))
        on = all(st(w)[0] == 'ON_STATION' and now - st(w)[1] < 10.0 for w in winners)
        ...
        if released and cur:
            ...
            self._emit(m.task_id, 'PERIMETER_FORMED', f'{len(winners)}/{len(winners)} on station - awaiting next action')
    ...
    def _run_auction(self, task_id, task_lat, task_lon):
        ...
        if task_id.startswith(('SWARM-', 'INTERCEPT-')) and len(bids) > 1:
            self.get_logger().info(f'{task_id}: SWARM task - assigning top {min(5, len(bids))} bidders to formation')
            winners = sorted(bids, key=bids.get)[:5]
            self.winners[task_id] = winners
            N = len(winners)
            for i, winner in enumerate(winners):
                # Pentagon ring offsets (meters)
                angle = (2 * math.pi * i) / N
                r = 300.0  # 150m ring radius
                ox, oy = r * math.cos(angle), r * math.sin(angle)
                ...
                res.path = [winner]
                res.execute_at = time.time() + 15.0  # T0: takeoff in 15s
                res.arrive_by = time.time() + 60.0   # T1: arrive in 60s
                res.offset_x = ox
                res.offset_y = oy
                res.orbit_radius = r
